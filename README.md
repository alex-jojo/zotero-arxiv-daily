...
- Optional AI-generated TL;DR for you to quickly pick up target papers. When LLM access is disabled, the original abstract is included instead.
- Paper affiliations are resolved and presented when LLM access is enabled.
...
Below are the supported secrets. The OpenAI-compatible LLM secrets are optional when `llm.enabled` is `false`. Secrets are hidden after they are saved.
...
| OPENAI_API_KEY | Optional. API key used only when `llm.enabled` is `true`. | sk-xxx |
| OPENAI_API_BASE | Optional. LLM API URL used only when `llm.enabled` is `true`. | https://api.openai.com/v1 |
...
llm:
  enabled: false # Set to true only after configuring an LLM API key and base URL.
  api:
    key: ${oc.env:OPENAI_API_KEY,null}
    base_url: ${oc.env:OPENAI_API_BASE,null}
...
llm:
  enabled: false # When false, no LLM API is called and emails contain the original abstracts.
  api:
    key: null # Required only when enabled is true. Example: sk-xxx
    base_url: null # Required only when enabled is true. Example: https://api.openai.com/v1
...
## 📖 How it works
*Zotero-arXiv-Daily* firstly retrieves all the papers in your Zotero library and all the papers released in the previous day, via corresponding API. Then it calculates the embedding of each paper's abstract via an embedding model. The score of a paper is its weighted average similarity over all your Zotero papers (newer paper added to the library has higher weight). When `llm.enabled` is true, the TLDR of each paper is generated by an LLM from the extracted text. Otherwise, the email includes the original abstract and does not call an LLM API.
